<?php
    session_start();
    include 'functions/functions-users.php';

    // check cookie
    if(!empty($_COOKIE["idUsers"]) && !empty($_COOKIE["usernameUsers"])){
        $query = "SELECT * FROM employee WHERE id=$_COOKIE[idUsers]";
    ...
    if(isset($_POST["login"])){
        if(login($_POST) > 0){
            ...
            header("Location: login-users.php?message=failed");
        }
        exit;
    }
?>
...
<body>
    ...
    <div>
        ...
        <div class="text-login">Belum mempunyai akun ? Silahkan buat akun di <a href="register-users.php"> REGISTER</a></div>
    </div>
